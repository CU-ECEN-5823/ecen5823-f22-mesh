 ...
 *         Removed the code that measures the duration of the button presses.
 *
 ******************************************************************************/
void sl_button_on_change(const sl_button_t *handle)
{
  //DOS unused now, uint32_t t_diff;
  ...
  for (uint8_t i = 0; i < SL_SIMPLE_BUTTON_COUNT; i++) {
    ...
    if (SL_SIMPLE_BUTTON_INSTANCE(i) == handle) {
      ...
      if (sl_button_get_state(handle) == SL_SIMPLE_BUTTON_PRESSED) {
        ...
        //DOS Not measuring the duration of button presses anymore.
        //DOS buttons[i].timestamp = sl_sleeptimer_get_tick_count();

        buttons[i].state = APP_BUTTON_PRESS_NONE; // Student edit required here
...
{
  (void) arguments; // DOS
  // DOS no need for this as far as I can tell.
  /*
  uint8_t button_id;
  uint8_t duration;
